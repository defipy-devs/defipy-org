---
title: "Overview"
description: "Core Primitives are the cross-protocol execution dispatchers that have been the recommended DeFiPy interface since v1.0. They handle pool initialization,…"
---
...
These primitives **mutate pool state** — they are execution operations (mint, burn, swap, zap). For read-only analytics (position analysis, risk assessment, price scenarios), see [agentic_primitive_contract](/primitive-contract/).
...
Core Primitives and [agentic_primitive_contract](/primitive-contract/) are both abstract — they dispatch across protocols without the caller knowing protocol details. The distinction is what they do:
...
For the full abstract interface API reference, see [abstract_uniswap](/abstract-uniswap/).
