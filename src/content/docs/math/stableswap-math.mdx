---
title: "Stableswap Math"
description: "Curve-style Stableswap 2-asset pools use an amplified invariant that interpolates between constant-product ($xy = k$) and constant-sum ($x + y = k$) behavior:"
---
...
DeFiPy's Stableswap math lives in [StableswapPy](https://github.com/defipy-devs/stableswappy). The amplified invariant, swap math, and `D` computation are implemented in `stableswappy.cpt.exchg.StableswapExchange`. The impermanent loss formula is implemented in `stableswappy.analytics.risk.StableswapImpLoss`.
...
:::note
Full derivation notebook (matching the depth of the Uniswap V2 and V3 math pages) is planned for a future release.
:::
