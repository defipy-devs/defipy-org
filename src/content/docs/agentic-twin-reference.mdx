...
The `pool_id` format for `LiveProvider` is `"<protocol>:<address>"`. V3 pools use the same shape: `"uniswap_v3:0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640"`. See [LiveProvider](/live-provider/) for the full surface — block pinning, decimal-adjusted reserves, V3 active-liquidity reads, and what's coming v2.2 (Balancer + Stableswap LiveProvider, V3 tick walking).
