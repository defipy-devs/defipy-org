...
v2.0 supports `"mcp"` only. `get_schemas("anthropic")` and `get_schemas("openai")` raise `NotImplementedError` with a v2.1 message. Both formats are derivable from MCP schemas with small envelope wrappers. See [Binding to Other LLMs](/binding-to-other-llms/).
