...
`defipy.tools` turns analytics primitives into MCP tool definitions. It emits JSON schemas, maintains the registry of what's exposed, and nothing else. Dispatch logic and pool resolution live in the MCP server (see [mcp_demo](/mcp-demo/)).
...
v2.0 supports `"mcp"` only. `get_schemas("anthropic")` and `get_schemas("openai")` raise `NotImplementedError` with a v2.1 message. Both formats are derivable from MCP schemas with small envelope wrappers. See [binding_to_other_llms](/binding-to-other-llms/).

For the full tool-schema walkthrough, see [tool_schemas](/agentic-tool-schemas/). For the curation roadmap, see [roadmap](/roadmap/).
