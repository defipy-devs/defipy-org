---
title: "The Primitive Interface"
description: "Every analytics primitive in DeFiPy follows the same three-line interface: stateless construction, computation at apply(), typed dataclass return."
---

Every analytics primitive in DeFiPy follows the same three-line interface:
...
- [Agentic Primitives](/agentic-primitives/) — the 9 categories, each listing the primitives that belong and their call signatures
- [Tools Reference](/agentic-tools-reference/) — `defipy.tools` module: schema emission, registry, binding
- [Twin Reference](/agentic-twin-reference/) — `defipy.twin` module: provider interface and snapshot types
- [Result Dataclasses](/agentic-result-dataclasses/) — the full catalog of result dataclasses returned by the primitives

:::note
"Primitive" here means an analytics primitive — `AnalyzePosition`, `CheckPoolHealth`, etc. The word is also used elsewhere in the docs at a lower layer to mean the protocol-bound exchange classes (`UniswapExchange`, `BalancerExchange`). Those live under [Protocol API → Uniswap V2](/api/protocol/uniswap-v2/) and siblings. The Agentic Primitives section is specifically about the stateless, typed, composable layer — the 21 classes exported from `defipy.primitives`.
:::
