---
title: "Result Dataclasses"
description: "Every agentic primitive returns a typed dataclass — no raw tuples, no dicts, no strings. Callers can rely on named fields."
---
...
:::note
Fields listed as "see source" will be expanded with full field documentation when autodoc is wired in a follow-up pass. The dataclasses whose fields are listed inline above are the most commonly used and were verified during the category notebook drafting.
:::

For the primitive that returns each dataclass, see [Agentic Primitives](/agentic-primitives/).
