...
description: "v2.0 ships MCP schemas only. Anthropic tool-use and OpenAI function-calling adapters are deferred to v2.3, derivable from the shipped MCP schemas."
---

v2.0 ships MCP schemas only. Anthropic tool-use and OpenAI function-calling adapters are deferred to v2.3 and will be derivable from the shipped MCP schemas with small wrappers.

`defipy.tools.get_schemas(format="anthropic")` and `get_schemas(format="openai")` currently raise `NotImplementedError`. The MCP format is strategically first for v2.0 because MCP is the direction the agent ecosystem is heading — Claude Desktop, Claude Code, and third-party MCP clients all consume it natively. Shipping DeFiPy as MCP-native is stronger positioning than emitting vendor-specific tool-use JSON.
...
2.  **Wait for v2.3.** Timeline tracked in the [Roadmap](/roadmap/).
