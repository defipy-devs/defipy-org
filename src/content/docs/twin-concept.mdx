---
title: "State Twin Concept"
description: "DeFi agents today are stuck in a posture they didn't choose. Every reasoning step has to wait for a chain confirmation. Every \"what if?\" requires an actual…"
---
...
![](/img/agent_without_substrate.svg)
...
The State Twin is the missing layer. It mirrors AMM state off-chain into a canonical, typed, replayable form. Every primitive in DeFiPy takes an `lp` argument — an exchange object that holds reserves, fees, tick state, weights, amplification coefficient, whatever the protocol needs. The State Twin is what builds that `lp` from a declarative snapshot, regardless of where the snapshot came from. `MockProvider` builds twins from canonical recipes for notebooks and demos. `LiveProvider` builds them from live chain reads (V2, V3, Balancer, and Stableswap). A custom provider can build them from a CSV file, a database row, a cached block, or a hypothetical scenario the analyst made up. The primitive can't tell the difference.

![](/img/state_twin_concept.svg)
...
- **Class**: `defipy.twin.StateTwinProvider`
  ...
  > - **Usage**: `class MyProvider(StateTwinProvider): def snapshot(self, pool_id: str, **kwargs) -> PoolSnapshot: ...`
...
- **Class**: `defipy.twin.LiveProvider`

  > - **Purpose**: Build twins from live chain reads (Uniswap V2, V3, Balancer, and Stableswap). Requires the `[chain]` install extra.
  > - **Usage**: `LiveProvider(rpc_url="https://...").snapshot("uniswap_v2:0xADDR")`
  > - **Status**: V2 and V3 shipped in v2.1; Balancer and Stableswap in v2.2 — all four read live from chain.
  > - **See**: [LiveProvider](/live-provider/) for the full surface — pool_id format, block pinning, decimal-adjusted reserves, V3 active-liquidity reads.
...
- **Class**: `defipy.twin.PoolSnapshot`
  ...
  > - **Purpose**: Declarative, protocol-agnostic representation of pool state. The discriminated union base class; subclasses (`V2PoolSnapshot`, `V3PoolSnapshot`, `BalancerPoolSnapshot`, `StableswapPoolSnapshot`) carry protocol-specific fields. As of v2.1, every snapshot carries optional chain-context fields (`block_number`, `timestamp`, `chain_id`), populated by `LiveProvider` and left as `None` by `MockProvider`.
...
- **Class**: `defipy.twin.StateTwinProvider`
  ...
  > - **Purpose**: Return a pool snapshot for a given pool identifier. `pool_id` semantics are provider-specific — a recipe name for MockProvider, a `"<protocol>:<address>"` string for LiveProvider.
  >
  > - **Methods**:  
  >   - `snapshot(pool_id: str, **kwargs) -> PoolSnapshot`  
  >     - **Parameters**:  
  >       - `pool_id`: Provider-specific identifier.
  >       - `**kwargs`: Provider-specific options. `LiveProvider` accepts `block_number=N` for historical pinning, `lwr_tick`/`upr_tick` for V3 tick range, and `n_coins` for Stableswap coin count; `MockProvider` silently absorbs unknown kwargs for cross-provider portability.
  >
...
A custom provider subclasses `StateTwinProvider` and implements `snapshot(pool_id, **kwargs)`. Same contract as MockProvider; different source of truth. This is the hook for fork state, cached blocks, or any snapshot source that isn't live chain (`LiveProvider`'s job) and isn't a canned recipe.
...
class CSVProvider(StateTwinProvider):
    ...
    def snapshot(self, pool_id: str, **kwargs):
