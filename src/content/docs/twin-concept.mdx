...
The State Twin is the missing layer. It mirrors AMM state off-chain into a canonical, typed, replayable form. Every primitive in DeFiPy takes an `lp` argument — an exchange object that holds reserves, fees, tick state, weights, amplification coefficient, whatever the protocol needs. The State Twin is what builds that `lp` from a declarative snapshot, regardless of where the snapshot came from. `MockProvider` builds twins from canonical recipes for notebooks and demos. `LiveProvider` builds them from live chain reads (V2 + V3). A custom provider can build them from a CSV file, a database row, a cached block, or a hypothetical scenario the analyst made up. The primitive can't tell the difference.
...
- **Class**: `defipy.twin.LiveProvider`
  ...
  > - **Purpose**: Build twins from live chain reads (Uniswap V2 + V3). Requires the `[chain]` install extra.
  > - **Usage**: `LiveProvider(rpc_url="https://...").snapshot("uniswap_v2:0xADDR")`
  > - **Status**: V2 + V3 ship in v2.1. Balancer + Stableswap LiveProviders are v2.2 work.
...
- **Class**: `defipy.twin.StateTwinProvider`
  ...
  >       - `**kwargs`: Provider-specific options. `LiveProvider` accepts `block_number=N` for historical pinning and `lwr_tick`/`upr_tick` for V3 tick range; `MockProvider` silently absorbs unknown kwargs for cross-provider portability.
  >
