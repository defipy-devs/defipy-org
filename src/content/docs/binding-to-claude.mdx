...
description: "DeFiPy v2 ships a reference MCP server at python/mcp/defipy_mcp_server.py that exposes the 10 curated tools over stdio transport. Wiring the server into…"
---
...
:::note
The v2.0 MCP server is read-only. No swaps, no signing, no plans — just analytics. Positions, entry amounts, and holding periods come from the user's question, not from chain state. Live chain reads ship in v2.1 via `LiveProvider` (see [twin_concept](/twin-concept/)).
:::
