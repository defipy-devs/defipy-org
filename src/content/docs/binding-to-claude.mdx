---
title: "Binding to Claude"
description: "DeFiPy v2 ships a reference MCP server that exposes the 10 curated tools over stdio transport. Wiring it into Claude Desktop or Claude Code takes one JSON edit and a restart."
---
...
:::note
The v2.0 MCP server is read-only. No swaps, no signing, no plans — just analytics. Positions, entry amounts, and holding periods come from the user's question, not from chain state. Live chain reads ship in v2.1 via `LiveProvider` (see [Twin Concept](/twin-concept/)).
:::
