...
description: "defipy.tools is the module that turns analytics primitives into MCP tool definitions. Its job is narrow: emit JSON schemas, maintain the registry of what's…"
---

`defipy.tools` is the module that turns analytics primitives into MCP tool definitions. Its job is narrow: emit JSON schemas, maintain the registry of what's exposed, and nothing else. There is no dispatch logic, no pool resolution, and no LLM-specific adapter code here — that all lives in the MCP server (see [mcp_demo](/mcp-demo/)).
...
v2.1+ candidate promotions as protocol parity and dependency blockers clear are tracked in [roadmap](/roadmap/).
