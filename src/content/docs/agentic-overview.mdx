...
- `defipy.twin` — a state-source abstraction (MockProvider for synthetic recipes, LiveProvider for V2, V3, Balancer, and Stableswap chain reads — V2/V3 since v2.1, Balancer/Stableswap since v2.2)
...
- [Tool Schemas](/agentic-tool-schemas/) — how `defipy.tools` turns a primitive into an MCP tool definition
- [Binding to Claude](/binding-to-claude/) — wiring the shipped MCP server into Claude Desktop and Claude Code
- [Binding to Other LLMs](/binding-to-other-llms/) — the same primitives, other tool-calling interfaces (v2.1)
- [MCP Demo](/mcp-demo/) — a walkthrough of the reference MCP server, including the full request/response trace from the verified session
...
:::note
The 10 primitives curated as MCP tools are a subset of the 21 shipped. Every shipped primitive is importable, documented, and tested — the curation is about what makes a good LLM tool, not what's "real." See [Agentic Primitives](/agentic-primitives/) for the full catalog.
:::
