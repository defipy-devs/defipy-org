...
**Protocol scope.** `CalculateSlippage` is V2/V3-only in v2.0 — calling it against a Balancer or Stableswap pool raises `ValueError`. The math is available in `BalancerExchange.get_amount_out` and `StableswapExchange.get_amount_out`; the gap is primitive packaging:
...
Once the Balancer/Stableswap extensions land (v2.1, Bucket A), [`CompareProtocols.slippage_at_amount`](/agentic-primitives/comparison/) closes its current `None` gap automatically — no API break expected.
