...
| Balancer | Deferred (v2.3) | Weighted-pool IL `α^w + (1-w)·α^(w-1) - 1` doesn't have a general closed-form inverse; Newton iteration converges cleanly but the symmetric upside/downside-α framing needs work. |
| Stableswap | Deferred (v2.3) | Fresh derivation needed over the invariant; mirrors `AssessDepegRisk`'s ε ↔ δ fixed-point but inverted. |
