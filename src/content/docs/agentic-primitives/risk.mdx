...
Risk is the most protocol-divergent category in v2.0 — what "risk" *means* differs structurally per AMM family. Rather than parallel siblings, the category fans out into protocol-specific primitives that name the right question for each AMM.
...
| Uniswap V3 | Full | `CheckTickRangeStatus` answers *"is my range still active?"* — the dominant V3-specific position risk. |
...
The category fans out by design — V3's risk is range-status, Stableswap's is depeg, Balancer's is weight exposure. Forcing them into a single primitive would conflate three structurally different questions.
