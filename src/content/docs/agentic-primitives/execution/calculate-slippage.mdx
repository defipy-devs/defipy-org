...
V2/V3 only — Balancer/Stableswap raise `ValueError` (deferred to v2.3, Bucket A).
...
| Balancer | ❌ Deferred (v2.3) — raises `ValueError` |
| Stableswap | ❌ Deferred (v2.3) — raises `ValueError` |
...
**Forward-look:** Once Bal/Ssw extensions land (v2.3, Bucket A), [`CompareProtocols.slippage_at_amount`](/agentic-primitives/comparison/compare-protocols/) closes its current `None` gap automatically — no API break expected.
