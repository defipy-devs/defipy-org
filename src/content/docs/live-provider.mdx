...
# Same shape as any twin — every primitive works against it
from defipy import CheckPoolHealth
health = CheckPoolHealth().apply(lp)
print(f"Spot price: {health.spot_price:.4f}")
```
...
# The twin feeds Stableswap-aware primitives the same way MockProvider does:
# AnalyzeStableswapPosition, AssessDepegRisk, SimulateStableswapPriceMove.
```

v2.2 covers **plain pools (2–3 coins)** — DAI/USDC/USDT 3pool is the canonical target. Rate-bearing pools (metapools, LSD pools) carry per-token rates and are out of scope.
