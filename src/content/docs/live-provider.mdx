---
title: "LiveProvider"
description: "Build State Twins from real on-chain pool state. Uniswap V2 and V3 (v2.1), Balancer and Stableswap (v2.2) — all four protocols read live from chain."
---
...
LiveProvider reads **Uniswap V2 and V3** (since v2.1) and **Balancer and Stableswap** (since v2.2) directly from chain. All four protocols produce snapshots that `StateTwinBuilder` turns into twins every primitive consumes the same way.
...
:::note
Both `[chain]` and `[agentic]` extras pin `web3 < 7.0` because `web3scout` (>= 1.0.0) uses a `web3._utils` API removed in web3 7. If web3 7.x is already installed for other reasons, the install will downgrade it. The web3 7.x migration is tracked upstream in web3scout.
:::
...
Balancer V2 weighted pools (2-asset) emit no per-pool balances of their own — they live on the canonical Vault, keyed by `poolId`. Pass the **WeightedPool** address; `LiveProvider` resolves the Vault and reads balances and normalized weights internally.
...
# Balancer 80/20 BAL/WETH weighted pool
snap = provider.snapshot(
...
# The snapshot carries the live pool state; lp is a BalancerExchange.
print(f"{snap.token0_name}/{snap.token1_name}  weights {snap.weight0}/{snap.weight1}")
print(f"reserves: {snap.reserve0:.4f} / {snap.reserve1:.4f}")

# Balancer-aware primitives take the twin from here — AnalyzeBalancerPosition,
# SimulateBalancerPriceMove, CompareProtocols. (CheckPoolHealth is V2/V3-only.)
```

v2.2 covers **2-asset weighted pools**. A pool with more than two tokens raises `NotImplementedError`.

## Quickstart — Stableswap

Curve-style plain Stableswap pools read per-pool — pass the pool address directly. The coin count is auto-detected; pass `n_coins` to skip the probe.
...
# Curve 3pool (DAI/USDC/USDT) — coin count auto-detected
snap = provider.snapshot(
...
print(f"coins:    {snap.token_names}")
print(f"reserves: {snap.reserves}")
print(f"A:        {snap.A}")
```

v2.2 covers **plain pools (2–3 coins)** — DAI/USDC/USDT 3pool is the canonical target. One boundary worth knowing: the **read path handles 2–3 coins, but the Stableswap position-analysis primitives (`AnalyzeStableswapPosition`, `AssessDepegRisk`) are 2-asset** — a 3pool twin reads and inspects fine, but those primitives expect a 2-coin pool. Rate-bearing pools (metapools, LSD pools) carry per-token rates and are out of scope.
...
| `balancer` | v2.2 (works — 2-asset weighted pools) |
| `stableswap` | v2.2 (works — plain pools, 2–3 coins) |
...
:::note
v2.1 V3 LiveProvider is **active-liquidity only**. It does not walk the tick bitmap — primitives that need liquidity at non-active ticks (`AssessLiquidityDepth`, `EvaluateTickRanges` in their full forms) will see empty tick data. Active-liquidity primitives (`AnalyzePosition`, `SimulatePriceMove`, `CheckPoolHealth`, `CalculateSlippage`, `DetectRugSignals`) work fully. Tick walking pairs with `AssessLiquidityDepth` in a future release.
:::
...
Balancer and Stableswap snapshots also batch through Multicall3, including the argument-bearing calls (`getPoolTokens(poolId)`, `coins(i)`, `balances(i)`) that V3's no-arg batch doesn't need. Balancer takes two pinned round trips — the pool's `poolId`/`vault`/weights, then the Vault's balances keyed by that `poolId` (the second can't start until the first returns). Stableswap takes one, after an optional coin-count probe.

The Multicall3 contract address (`0xcA11bde05977b3631167028862bE2a173976CA11`) is hardcoded; this address is the same on every major EVM chain Multicall3 has been deployed to. Chains without Multicall3 will fail with a descriptive error — multi-chain fallback is a future enhancement.
...
| `timestamp` | Block header timestamp (V2) or Multicall3's `getCurrentBlockTimestamp()` pinned to the resolved block (V3, Balancer, Stableswap) |
...
| `.snapshot(pool_id, *, block_number=None, lwr_tick=None, upr_tick=None, n_coins=None)` | `PoolSnapshot` | Read pool state and return a typed snapshot. `pool_id` is `"<protocol>:<address>"`. `block_number` defaults to `"latest"` (resolved once at the top of the call); pass an int for historical reads. `lwr_tick` / `upr_tick` apply to V3 pools only (default full-range); `n_coins` applies to Stableswap pools only (default: auto-detect via a coin-count probe). |
...
| Balancer pool with more than 2 tokens, or a non-plain (rate-bearing) Stableswap pool | `NotImplementedError` — v2.2 scope is 2-asset Balancer and plain Stableswap |
...
Balancer and Stableswap LiveProvider shipped in v2.2. Still ahead:
...
- **Planning primitives** — `PlanRebalance`, `PlanZapIn`, `PlanExit` (v2.3)

See the [Roadmap](/roadmap/) for the full timeline.
