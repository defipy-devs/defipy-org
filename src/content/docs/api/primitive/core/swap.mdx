...
Exchange one token for another at the pool's current price. `Swap` is a **mutating** dispatch primitive — it routes to the protocol-specific exact-input swap underneath the unified interface. See [The Primitive Interface](/primitive-interface/) for the cross-cutting invariants every dispatcher follows.
