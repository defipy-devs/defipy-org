---
title: "LPQuote"
description: "Read-only price, reserve, and LP↔token conversion queries. The recommended cross-protocol entry point for state reads — V2/V3 today, Balancer/Stableswap planned for v2.3."
---
...
| Balancer | 🔜 v2.3 — see "What's coming in v2.3" below |
| Stableswap | 🔜 v2.3 |
...
`LPQuote` works the same way against `LiveProvider`-built twins (i.e. real chain reads) as it does against `MockProvider` snapshots — the V2/V3 read path is identical regardless of how the twin was constructed.
