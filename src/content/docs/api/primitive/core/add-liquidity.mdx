...
Contribute liquidity to a pool that already has a price. `AddLiquidity` is a **mutating** dispatch primitive — every protocol accepts a single-token-in call shape, but the *meaning* of "what's calculated for you" varies. See [The Primitive Interface](/primitive-interface/) for the cross-cutting invariants every dispatcher follows.
