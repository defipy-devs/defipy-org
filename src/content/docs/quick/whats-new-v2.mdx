...
description: "From desktop tool to industrial infrastructure. v1 was built for the research analyst — a library you imported into a notebook. v2 is built for the agentic era."
---
...
The 21 stateless, typed analytics primitives are the core of v2 — all present since v1.2.0, now exposed as MCP tools without coupling DeFiPy to any specific LLM framework. The library itself remains pure analytics: zero LLM dependencies, zero network calls at core. Each primitive follows a three-line interface — stateless construction, computation at `.apply()`, typed dataclass return — making them callable identically from a notebook or from an LLM via MCP. They cover 9 categories of DeFi analytics:
...
10 of the 21 are curated as MCP tools for LLM consumption; all 21 are importable, documented, and tested. See the [Agentic Primitives Overview](/agentic-primitives/) for the full availability grid, primitive descriptions, and executable notebook examples for every category.

**defipy.tools**

Self-describing schemas for the 10 curated leaf primitives in [Model Context Protocol](https://modelcontextprotocol.io) (MCP) format. `get_schemas("mcp")` returns the schema list; `TOOL_REGISTRY` exposes the underlying `ToolSpec` entries for programmatic use. Anthropic tool-use and OpenAI function-calling formats shipped in v2.1 — both derived from the MCP schemas with small wrappers. See [Tool Schemas](/agentic-tool-schemas/) for the full module reference.

**defipy.twin**

The State Twin abstraction. `StateTwinProvider` defines the `snapshot(pool_id) → PoolSnapshot` contract. `MockProvider` ships four canonical synthetic recipes (`eth_dai_v2`, `eth_dai_v3`, `eth_dai_balancer_50_50`, `usdc_dai_stableswap_A10`). `LiveProvider` ships in v2.0 as a stub with a stable constructor; v2.1 made `snapshot()` functional for Uniswap V2 and V3, and v2.2 added Balancer and Stableswap. See [Twin Concept](/twin-concept/) for the full treatment.

**MCP server demo**

A reference stdio-transport server at `python/mcp/defipy_mcp_server.py` exposes DeFiPy's 10 curated tools to Claude Desktop, Claude Code, or any MCP client. Per-call receipts log as single-line JSON to stderr. See [Binding to Claude](/binding-to-claude/) for setup and [MCP Demo](/mcp-demo/) for a verified-live walkthrough.
...
- `LiveProvider` on-chain snapshots — Uniswap V2 and V3 in v2.1, Balancer and Stableswap in v2.2 (the ABC + stub shipped in v2.0)
- `defipy.observability` module (v2.1) — structured event sink with opt-in tracing, replacing the v2.0 stderr-receipt surface
- Anthropic tool-use and OpenAI function-calling schema formats (v2.1)
