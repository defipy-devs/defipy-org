...
description: "Agentic Primitives are stateless, typed analytics functions that answer questions about DeFi positions, pools, and risk. They follow a three-line contract:…"
---

Agentic Primitives are stateless, typed analytics functions that answer questions about DeFi positions, pools, and risk. They follow a three-line contract: stateless construction, computation at `.apply()`, typed dataclass return. Every primitive works identically whether called from a notebook or from an LLM via the MCP server. See [agentic_primitive_contract](/primitive-contract/) for the contract invariants.

These primitives **read pool state** — they are analytics operations (position decomposition, risk assessment, price scenarios). For execution operations that mutate pool state (mint, burn, swap, zap), see [core_primitives_index](/core-primitives/) under Core Primitives.
...
For the Solidity-derived protocol-specific classes underneath both layers, see [primitive_uniswapv2](/primitive-uniswapv2/) and siblings under Protocol Classes.
